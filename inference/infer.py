...
def main():
    ...
    for sample in range(50):
        _, _, samples[sample, :] = infer.inference(decoder, pos=batch_pos_tr[1, :600].reshape(
            1, -1), tar=batch_tar_tr[1, :50].reshape(1, -1), num_steps=550)

    samples[:, :50] = batch_tar_tr[1, :50]
    plt.style.use('ggplot')
    sorted_arr = np.argsort(batch_pos_tr[1, :])
    for i in range(4, 5):
        plt.plot(batch_pos_tr[1, sorted_arr], samples[i,
                                                      sorted_arr], 'lightsteelblue', alpha=0.6, zorder=-1)
    plt.plot(batch_pos_tr[1, sorted_arr], batch_tar_tr[1, sorted_arr], 'black')
    plt.scatter(batch_pos_tr[1, :50], batch_tar_tr[1, :50],
