...
def pick_diff_cols_from_each_row(arr, cols):
    ...
    In order to get more intuition run the following example in main:
...
def main():
    ...
    arr = np.random.choice(np.arange(1, 10), replace=True, size=(5, 10))
    print('arr: ', arr)
    cols = pd.DataFrame(arr).apply(lambda x: np.argsort(x), axis=1)
    print('after pick: ')
    print(pick_diff_cols_from_each_row(arr, cols))
