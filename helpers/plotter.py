...
def infer_plot(model, x_te, y_te, num_steps, sample_num, samples=10, mean=True, context_p=50):
    ...
    sorted_idx_part = pd.DataFrame(y_te[sample_num, context_p:]).apply(
        lambda x: np.argsort(x), axis=1)
    ...
    for inf in range(samples):
        _, pos_inf, tar_inf = infer.inference(model, x_te[sample_num, :maxi].reshape(
            1, -1), y_te[sample_num, :context_p].reshape(1, -1), num_steps=num_steps)
        axs.plot(pos_inf.reshape(-1)[sorted_idx].reshape(-1),
                 tar_inf.numpy().reshape(-1)[sorted_idx][0], c='lightskyblue')

    if mean:
        _, pos_inf, tar_inf = infer.inference(model, x_te[sample_num, :90].reshape(
            1, -1), y_te[sample_num, :context_p].reshape(1, -1), num_steps=num_steps, sample=False)
        axs.plot(pos_inf.reshape(-1)[sorted_idx].reshape(-1), tar_inf.numpy(
        ).reshape(-1)[sorted_idx][0], c='goldenrod', linewidth=2)
